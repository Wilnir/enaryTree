...
void encontrarMinMaxRec(PONT raiz, char* min, char* max){
   if (!raiz) return;
   PONT filho = raiz->ultimoFilho;
   PONT irmao = filho->proximoIrmao;
   while(filho && irmao != NULL){
   if(filho > raiz && raiz < irmao)
   printf("%c\n %c\n", irmao->chave, filho->chave);
   filho = filho->ultimoFilho;
   *max = filho->chave;
